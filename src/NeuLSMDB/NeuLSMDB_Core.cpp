...
#define GET_CACHE_LIST() static_cast<std::list<NeuLSMDB::CacheBlock> *>(_cacheList)
#define GET_CACHE_MAP() static_cast<std::map<uint64_t, std::list<NeuLSMDB::CacheBlock>::iterator> *>(_cacheMap)
...
NeuLSMDB::NeuLSMDB()
{
    ...
    _levelsLog = new std::vector<SSTFile>[MAX_LEVEL]();
...
bool NeuLSMDB::init()
{
    ...
    // Scan and reconstruct the volatile RAM metadata memory map for regular storage segments.
    loadAllSST();

    // ========================================================================
    // LOG RECOVERY SEQUENCING: RECONSTRUCT HISTORICAL LOG TOPOLOGY IN RAM
    // ========================================================================
    // Scan and mount all partitioned 'log_lvX_Y.sst' file blocks from non-volatile disk storage.
    // This populates the dedicated high-address log metadata index map during early boot initialization.
    loadAllSSTLog();

    // Reconstruct the remaining runtime transaction delta states left inside the volatile pool before a crash.
    // This sweeps the write-ahead log to reconstruct un-flushed regular and log records into the active RAM MemTable.
    replayWAL();
    ...
    if (_taskHandle == NULL)
    {
        // KERNEL DISPATCHER: Isolate low-priority structural reorganization tasks to physical CPU Core 1.
        // This offloads heavy multi-way merge compactions away from Core 0 to protect active application threads.
        xTaskCreatePinnedToCore(
            [](void *param)
            {
                ...
                for (;;)
                {
                    ...
                    db->tick();
                    vTaskDelay(pdMS_TO_TICKS(5)); // Relinquish CPU execution control window to prevent watchdog starvation
                }

                TaskHandle_t localHandle = db->_taskHandle;
                db->_taskHandle = NULL;
                vTaskDelete(NULL); // Terminate background process task context cleanly
            },
...
{
    // Execution Suspension Phase: Transition active pipeline states to safe boundaries
    // and broadcast a shutdown signal to the asynchronous worker thread.
    _systemReady = false;
    _compactState = IDLE;
    _compactLogState = IDLE;
    _stopTaskRequested = true;

    // Thread Synchronization Fencing: Enforce a strict timed wait interval to allow
    // the asynchronous task handle to exit gracefully on its own accord.
    int timeout = 0;
    ...
    // Hard Thread Purge Path: Forcefully delete the task context if it refuses
    // to exit inside the designated timing window to prevent memory hanging.
    if (_taskHandle != NULL)
    {
    ...
    // Pipeline Sanitization Phase: Reset active structural reorganization trackers for regular storage paths.
    if (_job != nullptr)
    {
        _job->active = false;
    ...
    // ========================================================================
    // Reset active structural reorganization trackers and close outstanding file descriptors
    // specifically belonging to the isolated log pipeline cascade.
    if (_jobLog != nullptr)
    {
    ...
    // Storage Destruction Sequence: Enumerate and recursively sweep the non-volatile disk directory.
    ...
    if (STORAGE_EXISTS("/lsm"))
    {
        ...
        // This systematically erases all immutable regular blocks and rolling log files simultaneously.
        for (const auto &filePath : listFileDelete)
        {
            STORAGE_REMOVE(filePath);
            vTaskDelay(pdMS_TO_TICKS(2)); // Introduce pacing delays to allow the virtual filesystem layout to settle
        }
        STORAGE_RMDIR("/lsm");
    }
    STORAGE_MKDIR("/lsm");
    ...
    // Clearing this unified container removes both regular records and log records instantly.
    auto memPtr = GET_MEM();
    if (memPtr != nullptr)
        memPtr->clear();

    // Level Index Wiping: Clear regular RAM vector indexes across all tree depths.
    ...
    // ========================================================================
    // Clear log RAM vector indexes across all tree depths to cleanly reset the isolated log pipeline track.
    ...
    // Acceleration Block Sanitization: ClearLeast-Recently-Used (LRU) cache lists and lookup registers.
    ...
    // Metric Invalidation Sequence: Enforce absolute sequential consistency while resetting atomic variables.
    ...
    // Engine Reboot: Trigger a cold system bootstrap sequence to re-mount fresh, blank directory paths.
    return init();
}
...
{
    // Defensive Range Validation: Instantly reject out-of-bounds keys before securing resource locks.
    if (!_systemReady || key >= NEU_KEY_SPACE_LIMIT)
        return false;

    // Mutex Acquisition: Acquire the unified system lock block to prevent dynamic multi-core race conditions.
    ...
    do
    {
        // Resource Capacity Guard: Intercept write cycles to execute reactive eviction parameters if limits breach constraints.
        ...
        // Write-Ahead Log (WAL) Serialization Channel: Commit the deletion cancellation frame to physical disk instantly.
        // Size parameter is locked strictly to 0, and the tombstone boolean flag parameter is forced to true.
        int retryWAL = 0;
        bool walSuccess = false;
        while (retryWAL < 10)
        {
            if (appendWAL(key, nullptr, 0, true)) // Ingesting (key, nullptr, size=0, tombstone=true)
            {
                walSuccess = true;
                break;
            }
            xSemaphoreGive(_mutex);
            vTaskDelay(pdMS_TO_TICKS(2)); // Back off control window to allow outstanding sector flushes to complete
            if (xSemaphoreTake(_mutex, pdMS_TO_TICKS(1000)) != pdTRUE)
                break;
            retryWAL++;
        }
        ...
        // Tombstone Frame Assignment: Construct an empty data node containing the cancellation marker.
        MemEntry tombEntry;
        tombEntry.size = 0;
        tombEntry.ts = now;
        ...
        if (it != mapMem.end())
        {
            // Mutation Path: Replace preexisting memory bytes allocation values and assign the tombstone node directly.
            _memBytes -= it->second.size;
            mapMem[key] = std::move(tombEntry);
        }
        else
        {
            // Allocation Path: Inject the brand new cancellation node into the volatile lookup tree map structure.
            mapMem[key] = std::move(tombEntry);
            __atomic_add_fetch(&_memCount, 1, __ATOMIC_SEQ_CST);
        }

        // Global Metrics Balancing: Safely decrement live active system metrics records counter tracking variables.
...
{
    // Concurrency Verification Phase: Read the current compaction pipeline phase atomically.
    // Early exit if any background structural data reorganization job is already active.
    CompactState st = __atomic_load_n(&_compactState, __ATOMIC_SEQ_CST);
    if (st != IDLE)
        return;

    // Hard Level-0 Density Barrier: Instantly trigger an immutable level merge if the Level 0 file count
    // exceeds the hard threshold. This directly curtails query lookup degradation caused by un-compacted file sprawl.
    ...
    // Heuristic Level Sizing Sweeper: Scan deep architectural tree tiers to evaluate volume imbalances.
    for (int lvl = 0; lvl < MAX_LEVEL - 1; lvl++)
    {
        // Exponential Threshold Strategy: Compute file density caps algorithmically using bitwise shifts (2^lvl).
        // This optimizes structural scaling parameters dynamically as data flows down into deeper levels.
        int thr = (lvl == 0) ? 2 : (2 << lvl);

        // Complex Boundary Invalidation Fence: Evaluate tiered capacity limits over regular storage partitions.
        // Once a specific tier breaches its file volume ceiling, a background compaction worker job is enqueued.
    ...
    do
    {
        ...
        for (int lvl = 0; lvl < MAX_LEVEL; lvl++)
        {
            ...
            for (auto itSST = level.rbegin(); itSST != level.rend(); ++itSST)
            {
                ...
                auto idxIt = std::lower_bound(sst.index.begin(), sst.index.end(), SSTIndex{(uint32_t)key, 0, 0, 0, false});
...
void NeuLSMDB::flush()
{
    ...
    // Concurrency Lock Acquisition: Acquire the unified database mutex resource lock.
    // This serializes the memory serialization pass to ensure atomic data transitions.
    if (xSemaphoreTake(_mutex, pdMS_TO_TICKS(1000)) != pdTRUE)
        return;
    ...
    // This forms the core backbone of our isolated Dual-Pipeline storage architecture.
    std::map<uint32_t, MemEntry> regulerEntries;
    std::map<uint32_t, MemEntry> logEntries;

    for (auto &kv : mapMem)
    {
        // Address Isolation Logic: Evaluate the key coordinate against the high-address offset baseline.
        // Keys above the offset are automatically routed to the historical rolling log track.
        if (kv.first >= NEU_LOG_KEY_OFFSET)
        {
            // Zero-Copy Optimization Pass: Move volatile pointers into the log pool via std::move
            // to eliminate expensive deep heap copy overhead operations during memory splitting.
    ...
    // Serialization Phase 1: Flush regular transactions down into immutable Level 0 regular tables.
    ...
    // Serialization Phase 2: Flush historical telemetry records down into isolated Level 0 log tables.
    if (!logEntries.empty())
    {
    ...
    // Fault-Tolerant Transaction Rollback Protocol: Protect memory states from sudden disk I/O errors.
    // If any underlying filesystem write sequence fails, reconstruct the original combined RAM map completely.
    if (!flushSuccess)
    {
        // Re-inject elements backwards using zero-copy moves to ensure no data is lost during structural failures.
    ...
    // Persistence Commit Synchronization: Clear volatile RAM trackers since all elements are now safely in flash disk storage.
    mapMem.clear();
    _memBytes = 0;
    __atomic_store_n(&_memCount, 0, __ATOMIC_SEQ_CST);

    // Close and truncate the old Write-Ahead Log (WAL) descriptor from the active storage layer.
    clearWAL();

    // Re-initialize an append-only file channel stream to log the next incoming transactional lifecycle sequence.
    _walFile = STORAGE_OPEN("/lsm/wal.log", FILE_APPEND);
    if (!_walFile)
    {
...
bool NeuLSMDB::appendWAL(uint16_t key, const void *data, size_t size, bool tombstone)
{
    if (!_walFile)
        return false;
...
uint32_t NeuLSMDB::getFileSeq()
{
    return _nextFileId++;
}

bool NeuLSMDB::writeSST(uint8_t level, const std::map<uint32_t, MemEntry> &entries, const String &dstFile)
{
    if (entries.empty())
        return true;
    ...
    // or generate a brand new Level 0 state block from our linear global unique identifier sequence.
    if (dstFile.length() > 0)
    {
        fn = dstFile;
    ...
    // Transaction Conversion Pass: Linear sweep across the map structure to execute binary block writes.
    for (auto &kv : entries)
    {
        uint32_t k = kv.first; // Implicit type alignment to match the custom 32-bit internal register coordinator.
        ...
        // Metadata Commit Phase: Direct byte write of fixed-length layout frames onto non-volatile target blocks.
        // If the file descriptor fails to write, instantly truncate the link to prevent storage fragmentation.
        ...
        // Payload Commit Phase: Stream raw data structures onto flash sectors directly trailing the header frame.
        ...
        // Memory Index Construction: Map runtime file-pointer bounds for fast lookups.
        // This hydatrates our active probabilistic bloom filter bitmask matrix concurrently during the write pass.
        SSTIndex si;
        si.key = k;
    ...
    // Index Standardization Pass: Sort the extracted tracking elements lexicographically by key
    // to satisfy strict binary search lookahead constraints inside our unified non-volatile read path layer.
...
bool NeuLSMDB::readSST(const SSTFile &sst, const SSTIndex &idxEntry, void *out, size_t &size)
{
...
__attribute__((always_inline)) inline void NeuLSMDB::internalDeleteSST(
    const std::vector<String> &files,
    std::vector<SSTFile> *levelsTarget)
{
    // Concurrency Lock Acquisition: Guard the metadata vector arrays from cross-core race conditions
    // by blocking outstanding application write requests until the tracking metadata is purged.
    if (_mutex && xSemaphoreTake(_mutex, portMAX_DELAY) == pdTRUE)
    {
        for (auto &fn : files)
        {
            // Level Hierarchy Sweep: Traverse across all deeper log/regular tree tiers
            // to find the exact target file pointer context in RAM.
            for (int l = 0; l < MAX_LEVEL; l++)
            {
                auto &currentLevel = levelsTarget[l];

                // Linear Lookahead Search: Locate the targeting SSTable object using filename comparison string queries.
                auto it = std::find_if(currentLevel.begin(), currentLevel.end(), [&](const SSTFile &x)
                                       { return x.filename == fn; });

                if (it != currentLevel.end())
                {
                    // Global Sizing Reconciliation Loop: Scan the internal file indices before deleting the node
                    // to reverse atomic system metrics accurately.
                    for (auto &e : it->index)
                    {
                        // Deduplication Count Guard: Only decrement active system metrics if the record
                        // does not possess a tombstone cancellation marker.
                        if (!e.tombstone)
                        {
                            size_t currentTotal = __atomic_load_n(&_totalEntryCount, __ATOMIC_SEQ_CST);
                            if (currentTotal > 0)
                    ...
                    // Metadata Eviction Phase: Cleanly erase the cached SSTable descriptor from the active RAM array map.
                    currentLevel.erase(it);
                    break;
                }
            }

            // Physical Unlinking Sequence: Trigger a virtual filesystem physical remove request.
            // This systematically frees up empty non-volatile flash or storage card blocks trailing compaction passes.
            STORAGE_REMOVE(fn);
        }
        xSemaphoreGive(_mutex);
    }
...
void NeuLSMDB::deleteSSTFiles(const std::vector<String> &files)
{
    ...
    // passing the dedicated regular storage file pipeline metadata array as the target.
...
void NeuLSMDB::cachePut(uint32_t fileId, uint32_t offset, const uint8_t *data, size_t len)
{
...
bool NeuLSMDB::cacheGet(uint32_t fileId, uint32_t offset, std::vector<uint8_t> &out)
{
    uint64_t k = makeCacheKey(fileId, offset);
    auto &mapC = *GET_CACHE_MAP();
    auto &listC = *GET_CACHE_LIST();
...
void NeuLSMDB::evictOldestData()
{
    ...
    // Sweep deep immutable levels in reverse order to find the absolute oldest historical metadata mutation.
    for (int lvl = MAX_LEVEL - 1; lvl >= 0; lvl--)
    {
        for (auto &sst : GET_LEVELS()[lvl])
        {
            ...
            {
                auto &e = sst.index[i];
                // Compare hardware timestamps to intercept and locate the most obsolete transaction node.
    ...
    // Phase 2: Scan Isolated Log Pipeline Track (Virtual High-Address Space Region)
    // Sweep matching log cascades to cross-evaluate data expiration states across both storage tracks.
    for (int lvl = MAX_LEVEL - 1; lvl >= 0; lvl--)
    {
    ...
    if (foundOldest)
    {
        ...
        // Commit an in-memory deletion marker directly at the target absolute location inside the active tree map.
        // This dynamically balances global counter states before the next scheduled flush routine.
        (*GET_MEM())[oldestKey] = std::move(delEntry);
        __atomic_sub_fetch(&_totalEntryCount, 1, __ATOMIC_SEQ_CST);
    }
    else
    {
        // Emergency Fallback Path: Hard file unlinking if volatile RAM metadata becomes blank under critical storage stress.
        // Direct non-volatile unlinking prevents physical disk saturation when memory lookup limits collapse.
...
void NeuLSMDB::auditLevels()
{
    ...
    Serial.printf("Active Records (Global Counter): %d | Capacity Limit: %d | Eviction Policy: %s\n",
    ...
    for (int lvl = 0; lvl < MAX_LEVEL; lvl++)
    {
        ...
        Serial.printf("    > Summary Level %d (Regular): Total Entries=%d | Tombstones=%d | Total Size: %d B\n",
    ...
    if (levelsLogPtr != nullptr)
    {
        for (int lvl = 0; lvl < MAX_LEVEL; lvl++)
        {
            ...
            Serial.printf("    > Summary Level %d (Log Pipeline): Total Entries=%d | Tombstones=%d | Total Size: %d B\n",
                          lvl, totalEntries, tombCount, totalSize);
        }
    }
